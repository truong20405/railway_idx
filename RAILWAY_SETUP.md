...
SESSION_RECOVERY_COOLDOWN=30
TAB_PRUNE_INTERVAL=180
...
- Login tuan tu theo thu tu account duoc chon:
  ...
  - Login account tiep theo (neu con slot).
...
- Neu recover browser that bai lien tiep, script cho `SESSION_RECOVERY_COOLDOWN` giay roi moi thu lai (tranh restart loop ngon CPU).
- Script tu dong dong tab du moi `TAB_PRUNE_INTERVAL` giay de han che RAM creep.
- Vong keepalive ngu `MAIN_LOOP_SLEEP` giay moi chu ky de giam CPU idle.
- Co the tu reboot chuong trinh moi `PROGRAM_RESTART_INTERVAL` giay (mac dinh 86400 = 24h), cong them jitter `PROGRAM_RESTART_JITTER`.
- `EXIT_ON_PROGRAM_RESTART=0` se reboot bang `exec` trong cung container (giu filesystem hien tai); set `1` de thoat process cho Railway restart service.
...
- De giu profile qua restart/redeploy tren Railway, gan persistent volume va dat `PROFILES_DIR` vao duong dan volume (vi du `/data/profiles`).
